...
class DensitometerPreprocessor:
    ...
    def extract_densitometer_data(
        ...
        # 필수 칼럼 확인 (수정됨: region_start/region_end → start_time/end_time)
        ...
        for idx, row in regions_df.iterrows():
            ...
            # 수정됨: region_start/region_end → start_time/end_time
            ...
            if control_type == 'controlled':
                ...
                if 'control_start' in row and not pd.isna(row['control_start']):
                    ...
                    # control_start가 없으면 중간 지점 사용
                    midpoint = start_time + (end_time - start_time) / 2
                    group_data['before/after'] = group_data['datetime'].apply(
                        lambda x: 'before' if x < midpoint else 'after'
                    )
            else:
                # 비제어 구간: reference_point 기준으로 before/after 구분
                if 'reference_point' in row and not pd.isna(row['reference_point']):
                    reference_point = self._parse_time(str(row['reference_point']))
                    group_data['before/after'] = group_data['datetime'].apply(
                        lambda x: 'before' if x < reference_point else 'after'
                    )
                else:
                    # reference_point가 없으면 중간 지점 사용
                    midpoint = start_time + (end_time - start_time) / 2
                    group_data['before/after'] = group_data['datetime'].apply(
                        lambda x: 'before' if x < midpoint else 'after'
                    )
